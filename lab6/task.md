...
![Фото 3](../assets/lab6/client-server-timings.jpg)
...
1. Скопировать папку сборки `public` в проект с бекендом:
   ![Public in backend](../assets/lab6/public-in-backend.png)
...
async function bootstrap() {
  ...
  app.useStaticAssets/lab6(resolve(__dirname, '..', 'public'));
...
-   `app.useStaticAssets/lab6(resolve(__dirname, '..', 'public'));` - указывает приложению путь к ассетам

-   Дополнительно нужно указать дженерик `NestExpressApplication` при создании экземляра приложения, чтобы ts понимал, что в качестве низкоуровневого фреймворка используется `express` (`useStaticAssets/lab6` как раз метод express-сервера)
...
![Frontend and backend](../assets/lab6/frontend-and-backend.png)

Итоговая структура приложения с бекендом из ЛР №4:
![Final backend](../assets/lab6/final-backend.png)
